...
??? note settings "Show Connection Strings settings"
...
??? note settings "Show Cookie settings"
    ...
    |---------|---------|-------------|
    | `ExpireDays` | 30 | Number of days before authentication cookie expires |

## Security Settings

??? note settings "Show Password Options"
...
??? note settings "Show Lockout Options"
...
??? note settings "Show Serilog base configuration"
...
??? note reference "Show all Serilog sinks (File / PostgreSQL / Console)"
    ...
    **PostgreSQL Sink** <span class="ee"></span>
...
??? note settings "Show other settings"
    ...
    | Setting | Default | Description |
    |---------|---------|-------------|
    | `DetailedErrors` | true | Show detailed error messages (set to `false` in production for security) |
    | `AllowedHosts` | * | Allowed host headers (* for all) |

## Example Complete Configuration

??? note examples "Show full appsettings.extra.json example"
