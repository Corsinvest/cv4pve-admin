...
public static class PveAdminHelper
{
    ...
    public static async Task<PveClient> GetPveClient(Configurations.ClusterOptions clusterOptions, ILogger logger)
    {
        try
        {
        ...
        {
            logger.LogError(ex, nameof(GetPveClient));
    ...
    public static async Task<ClusterStatus> GetClusterStatus(PveClient client)
        => (await client.Cluster.Status.Get()).First(a => a.Type == PveConstants.KeyApiCluster);

    public static string GetPveUrl(Configurations.ClusterOptions clusterOptions)
    {
        var client = ClientHelper.GetClientFromHA(clusterOptions.ApiHostsAndPortHA, 1000);
        return client == null
            ? throw new PveException("GetPveUrl error")
    ...
    public static string GetCategoryName(ModuleCategory category)
        => category switch
        {
            ModuleCategory.Control => "Control",
            ModuleCategory.Health => "Health",
            ModuleCategory.Management => "Management",
            ModuleCategory.Safe => "Safe",
            ModuleCategory.System => IModularityService.AdministrationCategoryName,
